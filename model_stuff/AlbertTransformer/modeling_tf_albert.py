...
from model_stuff.AlbertTransformer.configuration_bert import BertConfig
from model_stuff.AlbertTransformer.modeling_tf_utils import TFPreTrainedModel, get_initializer
from model_stuff.AlbertTransformer.file_utils import add_start_docstrings
...
class TFALBertAttention(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertAttention, self).__init__(**kwargs)
        self.self_attention = TFALBertSelfAttention(config, name='self')
        self.dense_output = TFALBertSelfOutput(config, name='output')
...
class TFALBertIntermediate(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertIntermediate, self).__init__(**kwargs)
...
class TFALBertOutput(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertOutput, self).__init__(**kwargs)
        self.dense = tf.keras.layers.Dense(config.hidden_size,
                                           kernel_initializer=get_initializer(config.initializer_range),
                                           name='dense')
        self.LayerNorm = tf.keras.layers.LayerNormalization(epsilon=config.layer_norm_eps, name='LayerNorm')
...
class TFALBertLayer(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.intermediate = TFALBertIntermediate(config, name='intermediate')
        self.bert_output = TFALBertOutput(config, name='output')
...
class TFALBertEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertEncoder, self).__init__(**kwargs)
        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.layer = [TFALBertLayer(config, name='layer_._{}'.format(i)) for i in range(config.num_hidden_layers)]
...
class TFALBertPooler(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertPooler, self).__init__(**kwargs)
        self.dense = tf.keras.layers.Dense(config.hidden_size,
                                           kernel_initializer=get_initializer(config.initializer_range),
                                           activation='tanh',
                                           name='dense')
...
class TFALBertPredictionHeadTransform(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertPredictionHeadTransform, self).__init__(**kwargs)
        self.dense = tf.keras.layers.Dense(config.hidden_size,
                                           kernel_initializer=get_initializer(config.initializer_range),
                                           name='dense')
...
class TFALBertLMPredictionHead(tf.keras.layers.Layer):
    def __init__(self, config, input_embeddings, **kwargs):
        super(TFALBertLMPredictionHead, self).__init__(**kwargs)
        self.vocab_size = config.vocab_size
        self.transform = TFALBertPredictionHeadTransform(config, name='transform')
    ...
    def build(self, input_shape):
        self.bias = self.add_weight(shape=(self.vocab_size,),
                                    ...
                                    trainable=True,
                                    name='bias')
        super(TFALBertLMPredictionHead, self).build(input_shape)
...
class TFALBertMLMHead(tf.keras.layers.Layer):
    def __init__(self, config, input_embeddings, **kwargs):
        ...
        self.predictions = TFALBertLMPredictionHead(config, input_embeddings, name='predictions')
...
class TFALBertNSPHead(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertNSPHead, self).__init__(**kwargs)
...
class TFALBertMainLayer(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        super(TFALBertMainLayer, self).__init__(**kwargs)
        self.num_hidden_layers = config.num_hidden_layers

        self.embeddings = TFALBertEmbeddings(config, name='embeddings')
        self.encoder = TFALBertEncoder(config, name='encoder')
        self.pooler = TFALBertPooler(config, name='pooler')
...
class TFALBertModel(TFALBertPreTrainedModel):
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFALBertModel, self).__init__(config, *inputs, **kwargs)
        self.bert = TFALBertMainLayer(config, name='bert')
...
class TFALBertForPreTraining(TFALBertPreTrainedModel):
    ...
    Examples::
        import tensorflow as tf
        from transformers import BertTokenizer, TFALBertForPreTraining
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForPreTraining.from_pretrained('bert-base-uncased')
        input_ids = tf.constant(tokenizer.encode("Hello, my dog is cute"))[None, :]  # Batch size 1
        outputs = model(input_ids)
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFALBertForPreTraining, self).__init__(config, *inputs, **kwargs)

        self.bert = TFALBertMainLayer(config, name='bert')
        self.nsp = TFALBertNSPHead(config, name='nsp___cls')
        self.mlm = TFALBertMLMHead(config, self.bert.embeddings, name='mlm___cls')
...
class TFALBertForMaskedLM(TFALBertPreTrainedModel):
    ...
    Examples::
        import tensorflow as tf
        from transformers import BertTokenizer, TFALBertForMaskedLM
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
...
@add_start_docstrings("""Bert Model with a `next sentence prediction (classification)` head on top. """,
                      BERT_START_DOCSTRING, BERT_INPUTS_DOCSTRING)
class TFALBertForNextSentencePrediction(TFALBertPreTrainedModel):
    r"""
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
    ...
    Examples::
        import tensorflow as tf
        from transformers import BertTokenizer, TFALBertForNextSentencePrediction
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForNextSentencePrediction.from_pretrained('bert-base-uncased')
        input_ids = tf.constant(tokenizer.encode("Hello, my dog is cute"))[None, :]  # Batch size 1
        outputs = model(input_ids)
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFALBertForNextSentencePrediction, self).__init__(config, *inputs, **kwargs)
...
class TFALBertForSequenceClassification(TFALBertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForSequenceClassification.from_pretrained('bert-base-uncased')
        input_ids = tf.constant(tokenizer.encode("Hello, my dog is cute"))[None, :]  # Batch size 1
        outputs = model(input_ids)
        logits = outputs[0]
    """
...
class TFALBertForMultipleChoice(TFALBertPreTrainedModel):
    ...
    Examples::
        import tensorflow as tf
        from transformers import BertTokenizer, TFALBertForMultipleChoice
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForMultipleChoice.from_pretrained('bert-base-uncased')
...
class TFALBertForTokenClassification(TFALBertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForTokenClassification.from_pretrained('bert-base-uncased')
        input_ids = tf.constant(tokenizer.encode("Hello, my dog is cute"))[None, :]  # Batch size 1
        outputs = model(input_ids)
        scores = outputs[0]
    """
    ...
    def __init__(self, config, *inputs, **kwargs):
        super(TFALBertForTokenClassification, self).__init__(config, *inputs, **kwargs)
        self.num_labels = config.num_labels
...
class TFALBertForQuestionAnswering(TFALBertPreTrainedModel):
    ...
    Examples::
        import tensorflow as tf
        from transformers import BertTokenizer, TFALBertForQuestionAnswering
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        model = TFALBertForQuestionAnswering.from_pretrained('bert-base-uncased')
        input_ids = tf.constant(tokenizer.encode("Hello, my dog is cute"))[None, :]  # Batch size 1
        outputs = model(input_ids)
