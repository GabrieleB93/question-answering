...
The three values for the learning rate have been tried and only the value $1e-5$ showed good convergence results for all the models that we compared (\Cref{fig:lr_005}), namely ALBERT, BERT base and BERT ``smart'' (trained using the loss introduced in \Cref{eq:loss}) with only the $3\%$ of unanswerable questions left and BERT base with the $6\%$ of unanswerable questions. 
...
\begin{figure}[ht]
	...
	\begin{subfigure}[t]{.45\textwidth}
		\centering
		\includegraphics[width=0.95\textwidth]{pics/acc_1_lr_005.pdf}
		\caption{Accuracy of start position values over the number of processed crops.}\label{fig:loss2}
	\end{subfigure}
        \vskip\baselineskip
	\begin{subfigure}[t]{.45\textwidth}
		\centering
